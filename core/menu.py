...
def render_menu(plugins_registry: Dict[str, Any]):
    ...
    for group_name in order:
        ...
        for item in items:
            ...
            print(f"         {C.GRAY}- {item.get('description', 'S/D')}{C.RESET}")
            print(f"         {fg_color}[{item['id']}]{C.RESET} {C.GRAY}Executar{C.RESET}")
            print(f"         {C.GRAY}---{C.RESET}")
        print() 
        
    print(f"{C.BRIGHT_RED}{C.BOLD} [0] {C.RESET} {C.GRAY}Encerrar Sessão{C.RESET}\n")
