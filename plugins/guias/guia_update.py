...
def run():
    print(f"\n{COLORS['bold']}{COLORS['yellow']}=== {METADATA['name']} ==={COLORS['reset']}")
    
    print(f"\n{COLORS['cyan']}[1] PREPARAÇÃO (ZONA LIMPA VS SUJA){COLORS['reset']}")
    ...
    print(f"\n{COLORS['cyan']}[2] MATRIZ DE DECISÃO DE SNAPSHOTS PROXMOX VE{COLORS['reset']}")
    print("  +----------------------+----------------------------------------------------------------+")
    ...
    print(f"\n{COLORS['cyan']}[3] ORQUESTRAÇÃO DO UPDATER (SOUP){COLORS['reset']}")
    ...
    print(f"\n{COLORS['red']}[!] ALERTA CRÍTICO DE AUTENTICAÇÃO: SALTSTACK (v2.4.210){COLORS['reset']}")
    ...
    print(f"\n{COLORS['cyan']}[4] PÓS-ATUALIZAÇÃO & CHECK-IN (PACIÊNCIA TÁTICA){COLORS['reset']}")
